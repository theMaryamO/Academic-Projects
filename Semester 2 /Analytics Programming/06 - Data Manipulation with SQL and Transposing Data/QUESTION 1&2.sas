...
*/
filename cwd 'C:\Users\fatima\OneDrive - De Montfort University\IMAT5168\week6';
...
data LOAN_RISK_AGE;
set LOAN_RISK;
if 15 <= age <= 19 then ageG = 'between 15 and 19';
else if 20 <= age <= 24 then ageG ='between 20 and 24';
else if 25 <= age <= 29 then ageG = 'between 25 and 29';
else if 30 <= age <= 34 then ageG = 'between 30 and 34';
else if 35 <= age <= 44 then ageG = 'between 35 and 44';
else if 45 <= age <= 64 then ageG = 'between 45 and 64';
else if 65 <= age <= 74 then ageG = 'between 65 and 74';
run; 

proc means
data = LOAN_RISK_AGE nway noprint;
class ageG;
var credits
	amount 
	duration;
output out =  LOAN_RISK_AGE (drop = _type_ _freq_)
sum(credits) = SUMcredits  
mean(amount) = MEANamount 
mean(duration) = AVGduration;
run;
 
proc sort data = LOAN_RISK_AGE;
by ageg;
run;
 proc print data = LOAN_RISK_AGE;
 run;

/*QUESTION 2*/
proc sql;
create table sqlsummary as  
    select (ageG) as ageG, 
