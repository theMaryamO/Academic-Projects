...
proc SQL;
title 'MERGED CREDIT, CUSTOMER AND ASSESSMENT';
create table LOAN_RISK2 as 
select CUSTOMER. *, when_assessed, rating
from CUSTOMER C left join ASSESSMENT A on C.customer=A.customer
right join RATING R on A.customer=R.customer;
quit;
...
format
	...
	;
run;
proc print data=LOAN_RISK2 label noobs;
run;


proc compare data = LOAN_RISK2 compare = work.LOAN_RISK;
run;
