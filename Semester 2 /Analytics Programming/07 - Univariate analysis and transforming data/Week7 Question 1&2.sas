...
/*QUESTION 1 - GROUP AGE*/ 
data LOANRISKAGE;
set LOAN_RISK;
if 15 <= age <= 19 then ageGroup = 'between 15 and 19';
else if 20 <= age <= 24 then ageGroup ='between 20 and 24';
else if 25 <= age <= 29 then ageGroup = 'between 25 and 29';
else if 30 <= age <= 34 then ageGroup = 'between 30 and 34';
else if 35 <= age <= 44 then ageGroup = 'between 35 and 44';
else if 45 <= age <= 64 then ageGroup = 'between 45 and 64';
else if 65 <= age <= 74 then ageGroup = 'between 65 and 74';
run; 
proc means
data = LOANRISKAGE mean stderr stddev kurtosis skewness p10 p90 clm q1 q3 nway noprint;
class ageGroup;
var   age 
	  amount 
	  duration  
      instalment;
output out =  LOANRISKAGE (drop = _type_ _freq_);
/*sum(credits) = SUMcredits  
mean(amount) = MEANamount 
mean(duration) = AVGduration;*/
run;
proc sort data = LOANRISKAGE;
by ageGroup;
run;
 proc print data = LOANRISKAGE;
 run;

/*QUESTION 2*/
proc SQL;
create table newage as  
    select (ageGroup) as ageGroup, 
