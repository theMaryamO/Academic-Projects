/*
	psa= 03
	requirement= import FITNESS data
	requirement= calculate new variables
...
These measurements were made on men involved in a physical fitness course at N.C.State Univ.
The variables are (units in brackets):
...
data FITNESS; 
	...
	input 
		...
		RunPulse 
		MaxPulse 
		@@				/* handles two data items per data line */
	; 
	datalines; 
...
run;
/* ratio compares how much restpulse compares to runpulse*/
data FITNESS;
set FITNESS;
restrun = restpulse/runpulse;
run;
data fitness;
set fitness;
...
/*
mile_time = runtime * 2/3;
if mile_time < 7 then quick = 1; else quick = 0;
drop mile_time;
*/

/*
if (runtime * 2/3) < 7 then quick = 1; else quick = 0;
*/

/* 
if quick =7 then quick =(runtime/1.5);
else quick =0;
put (restrun quick) ("");
*/
run;
